...
import net.prehistoricnaturefossils.client.model.ModelSkeletonPristerognathusFrame;
...
public class RenderFossilPristerognathus extends TileEntitySpecialRenderer<TileEntityFossilPristerognathus> {
    ...
    private static final ResourceLocation FRAME = new ResourceLocation(PrehistoricNatureFossils.MODID + ":textures/skeletons/pristerognathus_frame.png");

    private final ModelSkeletonPristerognathus modelSkeleton;
    private final ModelSkeletonPristerognathusFrame modelSkeletonFrame;

    public RenderFossilPristerognathus() {
        this.modelSkeletonFrame = new ModelSkeletonPristerognathusFrame();
    ...
    public void render(TileEntityFossilPristerognathus entity, double x, double y, double z, float partialTicks, int destroyStage, float alpha) {
        ...
        if (frame) {
            ...
            GlStateManager.enableAlpha();
            ModelSkeletonPristerognathusFrame modelSkeletonFrame = this.modelSkeletonFrame;
            scale = RenderPristerognathus.getScaler() * RenderDisplayWallMount.scaler;
            GlStateManager.translate(x + 0.5, y + 0.59, z + 0.5);
            GlStateManager.scale(scale,scale,scale);
            GlStateManager.rotate(180, 0F, 0F, 1F);
            GlStateManager.rotate(currentRotation, 0F, 1F, 0F);
