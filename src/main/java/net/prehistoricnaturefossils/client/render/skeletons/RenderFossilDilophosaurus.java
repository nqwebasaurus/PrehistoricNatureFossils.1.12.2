...
import net.prehistoricnaturefossils.client.model.ModelSkeletonDilophosaurusFrame;
...
public class RenderFossilDilophosaurus extends TileEntitySpecialRenderer<TileEntityFossilDilophosaurus> {
    ...
    private static final ResourceLocation FRAME = new ResourceLocation(PrehistoricNatureFossils.MODID + ":textures/skeletons/dilophosaurus_frame.png");

    private final ModelSkeletonDilophosaurus modelSkeleton;
    private final ModelSkeletonDilophosaurusFrame modelSkeletonFrame;

    public RenderFossilDilophosaurus() {
        this.modelSkeletonFrame = new ModelSkeletonDilophosaurusFrame();
    ...
    public void render(TileEntityFossilDilophosaurus entity, double x, double y, double z, float partialTicks, int destroyStage, float alpha) {
        ...
        if (frame) {
            ...
            GlStateManager.enableAlpha();
            ModelSkeletonDilophosaurusFrame modelSkeletonFrame = this.modelSkeletonFrame;
            scale = RenderDilophosaurus.getScaler() * RenderDisplayWallMount.scaler;
            GlStateManager.translate(x + 0.5, y + 1.45, z + 0.5);
            GlStateManager.scale(scale,scale,scale);
            GlStateManager.rotate(180, 0F, 0F, 1F);
            GlStateManager.rotate(currentRotation, 0F, 1F, 0F);
