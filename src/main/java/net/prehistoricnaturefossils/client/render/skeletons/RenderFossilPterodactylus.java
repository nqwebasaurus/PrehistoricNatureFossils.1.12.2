...
import net.prehistoricnaturefossils.client.model.ModelSkeletonPterodactylusFrame;
...
public class RenderFossilPterodactylus extends TileEntitySpecialRenderer<TileEntityFossilPterodactylus> {
    ...
    private static final ResourceLocation FRAME = new ResourceLocation(PrehistoricNatureFossils.MODID + ":textures/skeletons/pterodactylus_frame.png");

    private final ModelSkeletonPterodactylus modelSkeleton;
    private final ModelSkeletonPterodactylusFrame modelSkeletonFrame;

    public RenderFossilPterodactylus() {
        this.modelSkeletonFrame = new ModelSkeletonPterodactylusFrame();
    ...
    public void render(TileEntityFossilPterodactylus entity, double x, double y, double z, float partialTicks, int destroyStage, float alpha) {
        ...
        if (frame) {
            ...
            GlStateManager.enableAlpha();
            ModelSkeletonPterodactylusFrame modelSkeletonFrame = this.modelSkeletonFrame;
            scale = RenderPterodactylus.getScaler() * RenderDisplayWallMount.scaler;
            GlStateManager.translate(x + 0.5, y + 0.5, z + 0.5);
            GlStateManager.scale(scale,scale,scale);
            GlStateManager.rotate(180, 0F, 0F, 1F);
            GlStateManager.rotate(currentRotation, 0F, 1F, 0F);
