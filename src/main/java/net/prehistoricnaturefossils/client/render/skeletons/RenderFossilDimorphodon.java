...
import net.prehistoricnaturefossils.client.model.ModelSkeletonDimorphodonFrame;
...
public class RenderFossilDimorphodon extends TileEntitySpecialRenderer<TileEntityFossilDimorphodon> {
    ...
    private static final ResourceLocation FRAME = new ResourceLocation(PrehistoricNatureFossils.MODID + ":textures/skeletons/dimorphodon_frame.png");

    private final ModelSkeletonDimorphodon modelSkeleton;
    private final ModelSkeletonDimorphodonFrame modelSkeletonFrame;

    public RenderFossilDimorphodon() {
        this.modelSkeletonFrame = new ModelSkeletonDimorphodonFrame();
    ...
    public void render(TileEntityFossilDimorphodon entity, double x, double y, double z, float partialTicks, int destroyStage, float alpha) {
        ...
        if (frame) {
            ...
            GlStateManager.enableAlpha();
            ModelSkeletonDimorphodonFrame modelSkeletonFrame = this.modelSkeletonFrame;
            scale = RenderDimorphodon.getScaler() * RenderDisplayWallMount.scaler;
            GlStateManager.translate(x + 0.5, y + 0.44, z + 0.5);
            GlStateManager.scale(scale,scale,scale);
            GlStateManager.rotate(180, 0F, 0F, 1F);
            GlStateManager.rotate(currentRotation, 0F, 1F, 0F);
