...
import net.prehistoricnaturefossils.client.model.ModelSkeletonElginiaFrame;
...
public class RenderFossilElginia extends TileEntitySpecialRenderer<TileEntityFossilElginia> {
    ...
    private static final ResourceLocation FRAME = new ResourceLocation(PrehistoricNatureFossils.MODID + ":textures/skeletons/elginia_frame.png");

    private final ModelSkeletonElginia modelSkeleton;
    private final ModelSkeletonElginiaFrame modelSkeletonFrame;

    public RenderFossilElginia() {
        this.modelSkeletonFrame = new ModelSkeletonElginiaFrame();
    ...
    public void render(TileEntityFossilElginia entity, double x, double y, double z, float partialTicks, int destroyStage, float alpha) {
        ...
        if (frame) {
            ...
            GlStateManager.enableAlpha();
            ModelSkeletonElginiaFrame modelSkeletonFrame = this.modelSkeletonFrame;
            scale = RenderElginia.getScaler() * RenderDisplayWallMount.scaler;
            GlStateManager.translate(x + 0.5, y + 0.6, z + 0.5);
            GlStateManager.scale(scale,scale,scale);
            GlStateManager.rotate(180, 0F, 0F, 1F);
            GlStateManager.rotate(currentRotation, 0F, 1F, 0F);
