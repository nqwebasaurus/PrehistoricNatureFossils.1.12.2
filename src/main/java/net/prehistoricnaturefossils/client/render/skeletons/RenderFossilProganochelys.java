...
import net.prehistoricnaturefossils.client.model.ModelSkeletonProganochelysFrame;
...
public class RenderFossilProganochelys extends TileEntitySpecialRenderer<TileEntityFossilProganochelys> {
    ...
    private static final ResourceLocation FRAME = new ResourceLocation(PrehistoricNatureFossils.MODID + ":textures/skeletons/proganochelys_frame.png");

    private final ModelSkeletonProganochelys modelSkeleton;
    private final ModelSkeletonProganochelysFrame modelSkeletonFrame;

    public RenderFossilProganochelys() {
        this.modelSkeletonFrame = new ModelSkeletonProganochelysFrame();
    ...
    public void render(TileEntityFossilProganochelys entity, double x, double y, double z, float partialTicks, int destroyStage, float alpha) {
        ...
        if (frame) {
            ...
            GlStateManager.enableAlpha();
            ModelSkeletonProganochelysFrame modelSkeletonFrame = this.modelSkeletonFrame;
            scale = RenderProganochelys.getScaler() * RenderDisplayWallMount.scaler;
            GlStateManager.translate(x + 0.5, y + 0.91, z + 0.5);
            GlStateManager.scale(scale,scale,scale);
            GlStateManager.rotate(180, 0F, 0F, 1F);
            GlStateManager.rotate(currentRotation, 0F, 1F, 0F);
