...
import net.prehistoricnaturefossils.client.model.ModelSkeletonSuminiaFrame;
...
public class RenderFossilSuminia extends TileEntitySpecialRenderer<TileEntityFossilSuminia> {
    ...
    private static final ResourceLocation FRAME = new ResourceLocation(PrehistoricNatureFossils.MODID + ":textures/skeletons/suminia_frame.png");

    private final ModelSkeletonSuminia modelSkeleton;
    private final ModelSkeletonSuminiaFrame modelSkeletonFrame;

    public RenderFossilSuminia() {
        this.modelSkeletonFrame = new ModelSkeletonSuminiaFrame();
    ...
    public void render(TileEntityFossilSuminia entity, double x, double y, double z, float partialTicks, int destroyStage, float alpha) {
        ...
        if (frame) {
            ...
            GlStateManager.enableAlpha();
            ModelSkeletonSuminiaFrame modelSkeletonFrame = this.modelSkeletonFrame;
            scale = RenderSuminia.getScaler() * RenderDisplayWallMount.scaler;
            GlStateManager.translate(x + 0.5, y + 0.21, z + 0.5);
            GlStateManager.scale(scale,scale,scale);
            GlStateManager.rotate(180, 0F, 0F, 1F);
            GlStateManager.rotate(currentRotation, 0F, 1F, 0F);
