...
import net.prehistoricnaturefossils.client.model.ModelSkeletonThecodontosaurusFrame;
...
public class RenderFossilThecodontosaurus extends TileEntitySpecialRenderer<TileEntityFossilThecodontosaurus> {
    ...
    private static final ResourceLocation FRAME = new ResourceLocation(PrehistoricNatureFossils.MODID + ":textures/skeletons/thecodontosaurus_frame.png");

    private final ModelSkeletonThecodontosaurus modelSkeleton;
    private final ModelSkeletonThecodontosaurusFrame modelSkeletonFrame;

    public RenderFossilThecodontosaurus() {
        this.modelSkeletonFrame = new ModelSkeletonThecodontosaurusFrame();
    ...
    public void render(TileEntityFossilThecodontosaurus entity, double x, double y, double z, float partialTicks, int destroyStage, float alpha) {
        ...
        if (frame) {
            ...
            GlStateManager.enableAlpha();
            ModelSkeletonThecodontosaurusFrame modelSkeletonFrame = this.modelSkeletonFrame;
            scale = RenderThecodontosaurus.getScaler() * RenderDisplayWallMount.scaler;
            GlStateManager.translate(x + 0.5, y + 0.67, z + 0.5);
            GlStateManager.scale(scale,scale,scale);
            GlStateManager.rotate(180, 0F, 0F, 1F);
            GlStateManager.rotate(currentRotation, 0F, 1F, 0F);
