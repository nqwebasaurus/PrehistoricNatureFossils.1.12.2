...
import net.prehistoricnaturefossils.client.model.ModelSkeletonTetraceratopsFrame;
...
public class RenderFossilTetraceratops extends TileEntitySpecialRenderer<TileEntityFossilTetraceratops> {
    ...
    private static final ResourceLocation FRAME = new ResourceLocation(PrehistoricNatureFossils.MODID + ":textures/skeletons/tetraceratops_frame.png");

    private final ModelSkeletonTetraceratops modelSkeleton;
    private final ModelSkeletonTetraceratopsFrame modelSkeletonFrame;

    public RenderFossilTetraceratops() {
        this.modelSkeletonFrame = new ModelSkeletonTetraceratopsFrame();
    ...
    public void render(TileEntityFossilTetraceratops entity, double x, double y, double z, float partialTicks, int destroyStage, float alpha) {
        ...
        if (frame) {
            ...
            GlStateManager.enableAlpha();
            ModelSkeletonTetraceratopsFrame modelSkeletonFrame = this.modelSkeletonFrame;
            scale = RenderTetraceratops.getScaler() * RenderDisplayWallMount.scaler;
            GlStateManager.translate(x + 0.5, y + 0.43, z + 0.5);
            GlStateManager.scale(scale,scale,scale);
            GlStateManager.rotate(180, 0F, 0F, 1F);
            GlStateManager.rotate(currentRotation, 0F, 1F, 0F);
