...
import net.prehistoricnaturefossils.client.model.ModelSkeletonRubidgeaFrame;
...
public class RenderFossilRubidgea extends TileEntitySpecialRenderer<TileEntityFossilRubidgea> {
    ...
    private static final ResourceLocation FRAME = new ResourceLocation(PrehistoricNatureFossils.MODID + ":textures/skeletons/rubidgea_frame.png");

    private final ModelSkeletonRubidgea modelSkeleton;
    private final ModelSkeletonRubidgeaFrame modelSkeletonFrame;

    public RenderFossilRubidgea() {
        this.modelSkeletonFrame = new ModelSkeletonRubidgeaFrame();
    ...
    public void render(TileEntityFossilRubidgea entity, double x, double y, double z, float partialTicks, int destroyStage, float alpha) {
        ...
        if (frame) {
            ...
            GlStateManager.enableAlpha();
            ModelSkeletonRubidgeaFrame modelSkeletonFrame = this.modelSkeletonFrame;
            scale = RenderRubidgea.getScaler() * RenderDisplayWallMount.scaler;
            GlStateManager.translate(x + 0.5, y + 0.76, z + 0.5);
            GlStateManager.scale(scale,scale,scale);
            GlStateManager.rotate(180, 0F, 0F, 1F);
            GlStateManager.rotate(currentRotation, 0F, 1F, 0F);
