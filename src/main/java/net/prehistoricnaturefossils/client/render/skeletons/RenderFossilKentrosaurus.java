...
import net.prehistoricnaturefossils.client.model.ModelSkeletonKentrosaurusFrame;
...
public class RenderFossilKentrosaurus extends TileEntitySpecialRenderer<TileEntityFossilKentrosaurus> {
    ...
    private static final ResourceLocation FRAME = new ResourceLocation(PrehistoricNatureFossils.MODID + ":textures/skeletons/kentrosaurus_frame.png");

    private final ModelSkeletonKentrosaurus modelSkeleton;
    private final ModelSkeletonKentrosaurusFrame modelSkeletonFrame;

    public RenderFossilKentrosaurus() {
        this.modelSkeletonFrame = new ModelSkeletonKentrosaurusFrame();
    ...
    public void render(TileEntityFossilKentrosaurus entity, double x, double y, double z, float partialTicks, int destroyStage, float alpha) {
        ...
        if (frame) {
            ...
            GlStateManager.enableAlpha();
            ModelSkeletonKentrosaurusFrame modelSkeletonFrame = this.modelSkeletonFrame;
            scale = RenderKentrosaurus.getScaler() * RenderDisplayWallMount.scaler;
            GlStateManager.translate(x + 0.5, y + 1.15, z + 0.5);
            GlStateManager.scale(scale,scale,scale);
            GlStateManager.rotate(180, 0F, 0F, 1F);
            GlStateManager.rotate(currentRotation, 0F, 1F, 0F);
