...
import net.prehistoricnaturefossils.client.model.ModelSkeletonProburnetiaFrame;
...
public class RenderFossilProburnetia extends TileEntitySpecialRenderer<TileEntityFossilProburnetia> {
    ...
    private static final ResourceLocation FRAME = new ResourceLocation(PrehistoricNatureFossils.MODID + ":textures/skeletons/proburnetia_frame.png");

    private final ModelSkeletonProburnetia modelSkeleton;
    private final ModelSkeletonProburnetiaFrame modelSkeletonFrame;

    public RenderFossilProburnetia() {
        this.modelSkeletonFrame = new ModelSkeletonProburnetiaFrame();
    ...
    public void render(TileEntityFossilProburnetia entity, double x, double y, double z, float partialTicks, int destroyStage, float alpha) {
        ...
        if (frame) {
            ...
            GlStateManager.enableAlpha();
            ModelSkeletonProburnetiaFrame modelSkeletonFrame = this.modelSkeletonFrame;
            scale = RenderProburnetia.getScaler() * RenderDisplayWallMount.scaler;
            GlStateManager.translate(x + 0.5, y + 0.56, z + 0.5);
            GlStateManager.scale(scale,scale,scale);
            GlStateManager.rotate(180, 0F, 0F, 1F);
            GlStateManager.rotate(currentRotation, 0F, 1F, 0F);
