...
import net.prehistoricnaturefossils.client.model.ModelSkeletonMixosaurusFrame;
...
public class RenderFossilMixosaurus extends TileEntitySpecialRenderer<TileEntityFossilMixosaurus> {
    ...
    private static final ResourceLocation FRAME = new ResourceLocation(PrehistoricNatureFossils.MODID + ":textures/skeletons/mixosaurus_frame.png");

    private final ModelSkeletonMixosaurus modelSkeleton;
    private final ModelSkeletonMixosaurusFrame modelSkeletonFrame;

    public RenderFossilMixosaurus() {
        this.modelSkeletonFrame = new ModelSkeletonMixosaurusFrame();
    ...
    public void render(TileEntityFossilMixosaurus entity, double x, double y, double z, float partialTicks, int destroyStage, float alpha) {
        ...
        if (frame) {
            ...
            GlStateManager.enableAlpha();
            ModelSkeletonMixosaurusFrame modelSkeletonFrame = this.modelSkeletonFrame;
            scale = RenderMixosaurus.getScaler() * RenderDisplayWallMount.scaler;
            GlStateManager.translate(x + 0.5, y + 0.9, z + 0.5);
            GlStateManager.scale(scale,scale,scale);
            GlStateManager.rotate(180, 0F, 0F, 1F);
            GlStateManager.rotate(currentRotation, 0F, 1F, 0F);
