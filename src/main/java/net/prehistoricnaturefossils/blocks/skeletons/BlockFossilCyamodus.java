...
public class BlockFossilCyamodus extends BlockSkeletonBase {
    ...
    public void addInformation(ItemStack stack, World player, List<String> tooltip, ITooltipFlag advanced) {
        tooltip.add("When completed contains " + new TileEntityFossilCyamodus().getStages() + " part(s)");
    ...
    public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
        ...
        if (playerIn.getHeldItemMainhand().getItem() == Item.getItemFromBlock(this)) {
            if (!worldIn.isRemote) {
                ...
                if (tileEntity != null) {
                    if (tileEntity instanceof TileEntityFossilCyamodus) {
                        if (((TileEntityFossilCyamodus) tileEntity).getStages() >= newStage) {
                            ...
                            if (newStage == ((TileEntityFossilCyamodus) tileEntity).getStages()) {
                                if (playerIn instanceof EntityPlayerMP) {
                                    ModTriggers.CYAMODUS_COMPLETE.trigger((EntityPlayerMP) playerIn);
                                }
                            }
                            return true;
                        }
