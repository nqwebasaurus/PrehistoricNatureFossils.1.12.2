...
public class BlockFossilDakosaurus extends BlockSkeletonBase {
    ...
    public void addInformation(ItemStack stack, World player, List<String> tooltip, ITooltipFlag advanced) {
        tooltip.add("When completed contains " + new TileEntityFossilDakosaurus().getStages() + " part(s)");
    ...
    public TileEntity createTileEntity(World world, IBlockState state) {
        return new TileEntityFossilDakosaurus();
    }
    ...
    public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
        ...
        if (playerIn.getHeldItemMainhand().getItem() == Item.getItemFromBlock(this)) {
            if (!worldIn.isRemote) {
                ...
                if (tileEntity != null) {
                    if (tileEntity instanceof TileEntityFossilDakosaurus) {
                        if (((TileEntityFossilDakosaurus) tileEntity).getStages() >= newStage) {
                            ...
                            if (newStage == ((TileEntityFossilDakosaurus) tileEntity).getStages()) {
                                if (playerIn instanceof EntityPlayerMP) {
                                    ModTriggers.DAKOSAURUS_COMPLETE.trigger((EntityPlayerMP) playerIn);
                                }
                            }
                            return true;
                        }
