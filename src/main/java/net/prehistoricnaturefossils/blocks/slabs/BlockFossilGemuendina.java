...
public class BlockFossilGemuendina extends BlockSlabBase {

    public BlockFossilGemuendina() {
        setRegistryName(PrehistoricNatureFossils.MODID, "skeleton_gemuendina");
        setTranslationKey("pf_skeleton_gemuendina");
        GameRegistry.registerTileEntity(TileEntityFossilGemuendina.class, PrehistoricNatureFossils.MODID + ":tileentityskeleton_gemuendina");
    ...
    public String getDNACompat() {
        return "mobdnaPNlepidodendron:prehistoric_flora_gemuendina";
    }

    @SideOnly(Side.CLIENT)
    @Override
    public void addInformation(ItemStack stack, World player, List<String> tooltip, ITooltipFlag advanced) {
        tooltip.add("When completed contains " + new TileEntityFossilGemuendina().getStages() + " part(s)");
    ...
    public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
        ...
        if (playerIn.getHeldItemMainhand().getItem() == Item.getItemFromBlock(this)) {
            if (!worldIn.isRemote) {
                ...
                if (tileEntity != null) {
                    if (tileEntity instanceof TileEntityFossilGemuendina) {
                        if (((TileEntityFossilGemuendina) tileEntity).getStages() >= newStage) {
