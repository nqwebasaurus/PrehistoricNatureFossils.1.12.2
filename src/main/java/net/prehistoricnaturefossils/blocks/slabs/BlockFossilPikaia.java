...
public class BlockFossilPikaia extends BlockSlabBase {

    public BlockFossilPikaia() {
        setRegistryName(PrehistoricNatureFossils.MODID, "skeleton_pikaia");
        setTranslationKey("pf_skeleton_pikaia");
        GameRegistry.registerTileEntity(TileEntityFossilPikaia.class, PrehistoricNatureFossils.MODID + ":tileentityskeleton_pikaia");
    ...
    public String getDNACompat() {
        return "mobdnaPNlepidodendron:prehistoric_flora_pikaia";
    }

    @SideOnly(Side.CLIENT)
    @Override
    public void addInformation(ItemStack stack, World player, List<String> tooltip, ITooltipFlag advanced) {
        tooltip.add("When completed contains " + new TileEntityFossilPikaia().getStages() + " part(s)");
    ...
    public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
        ...
        if (playerIn.getHeldItemMainhand().getItem() == Item.getItemFromBlock(this)) {
            if (!worldIn.isRemote) {
                ...
                if (tileEntity != null) {
                    if (tileEntity instanceof TileEntityFossilPikaia) {
                        if (((TileEntityFossilPikaia) tileEntity).getStages() >= newStage) {
