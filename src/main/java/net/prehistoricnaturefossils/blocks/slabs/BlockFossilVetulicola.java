...
public class BlockFossilVetulicola extends BlockSlabBase {

    public BlockFossilVetulicola() {
        setRegistryName(PrehistoricNatureFossils.MODID, "skeleton_vetulicola");
        setTranslationKey("pf_skeleton_vetulicola");
        GameRegistry.registerTileEntity(TileEntityFossilVetulicola.class, PrehistoricNatureFossils.MODID + ":tileentityskeleton_vetulicola");
    ...
    public String getDNACompat() {
        return "mobdnaPNlepidodendron:prehistoric_flora_vetulicola";
    }

    @SideOnly(Side.CLIENT)
    @Override
    public void addInformation(ItemStack stack, World player, List<String> tooltip, ITooltipFlag advanced) {
        tooltip.add("When completed contains " + new TileEntityFossilVetulicola().getStages() + " part(s)");
    ...
    public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
        ...
        if (playerIn.getHeldItemMainhand().getItem() == Item.getItemFromBlock(this)) {
            if (!worldIn.isRemote) {
                ...
                if (tileEntity != null) {
                    if (tileEntity instanceof TileEntityFossilVetulicola) {
                        if (((TileEntityFossilVetulicola) tileEntity).getStages() >= newStage) {
