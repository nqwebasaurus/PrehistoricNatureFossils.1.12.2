...
public class BlockFossilMaclurina extends BlockSlabBase {

    public BlockFossilMaclurina() {
        setRegistryName(PrehistoricNatureFossils.MODID, "skeleton_maclurina");
        setTranslationKey("pf_skeleton_maclurina");
        GameRegistry.registerTileEntity(TileEntityFossilMaclurina.class, PrehistoricNatureFossils.MODID + ":tileentityskeleton_maclurina");
    ...
    public String getDNACompat() {
        return "mobdnaPNlepidodendron:prehistoric_flora_maclurina";
    }

    @SideOnly(Side.CLIENT)
    @Override
    public void addInformation(ItemStack stack, World player, List<String> tooltip, ITooltipFlag advanced) {
        tooltip.add("When completed contains " + new TileEntityFossilMaclurina().getStages() + " part(s)");
    ...
    public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
        ...
        if (playerIn.getHeldItemMainhand().getItem() == Item.getItemFromBlock(this)) {
            if (!worldIn.isRemote) {
                ...
                if (tileEntity != null) {
                    if (tileEntity instanceof TileEntityFossilMaclurina) {
                        if (((TileEntityFossilMaclurina) tileEntity).getStages() >= newStage) {
