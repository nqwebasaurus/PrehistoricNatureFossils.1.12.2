...
public class BlockFossilFalcatus extends BlockSlabBase {

    public BlockFossilFalcatus() {
        setRegistryName(PrehistoricNatureFossils.MODID, "skeleton_falcatus");
        setTranslationKey("pf_skeleton_falcatus");
        GameRegistry.registerTileEntity(TileEntityFossilFalcatus.class, PrehistoricNatureFossils.MODID + ":tileentityskeleton_falcatus");
    ...
    public void addInformation(ItemStack stack, World player, List<String> tooltip, ITooltipFlag advanced) {
        tooltip.add("When completed contains " + new TileEntityFossilFalcatus().getStages() + " part(s)");
    ...
    public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
        ...
        if (playerIn.getHeldItemMainhand().getItem() == Item.getItemFromBlock(this)) {
            if (!worldIn.isRemote) {
                ...
                if (tileEntity != null) {
                    if (tileEntity instanceof TileEntityFossilFalcatus) {
                        if (((TileEntityFossilFalcatus) tileEntity).getStages() >= newStage) {
