...
public class BlockFossilLimulid extends BlockSlabBase {

    public BlockFossilLimulid() {
        setRegistryName(PrehistoricNatureFossils.MODID, "skeleton_limulid");
        setTranslationKey("pf_skeleton_limulid");
        GameRegistry.registerTileEntity(TileEntityFossilLimulid.class, PrehistoricNatureFossils.MODID + ":tileentityskeleton_limulid");
    ...
    public String getDNACompat() {
        return "mobdnaPNlepidodendron:prehistoric_flora_limulid";
    }

    @SideOnly(Side.CLIENT)
    @Override
    public void addInformation(ItemStack stack, World player, List<String> tooltip, ITooltipFlag advanced) {
        tooltip.add("When completed contains " + new TileEntityFossilLimulid().getStages() + " part(s)");
    ...
    public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
        ...
        if (playerIn.getHeldItemMainhand().getItem() == Item.getItemFromBlock(this)) {
            if (!worldIn.isRemote) {
                ...
                if (tileEntity != null) {
                    if (tileEntity instanceof TileEntityFossilLimulid) {
                        if (((TileEntityFossilLimulid) tileEntity).getStages() >= newStage) {
