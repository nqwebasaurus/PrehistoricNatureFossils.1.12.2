...
public class BlockFossilMegarachne extends BlockSlabBase {

    public BlockFossilMegarachne() {
        setRegistryName(PrehistoricNatureFossils.MODID, "skeleton_megarachne");
        setTranslationKey("pf_skeleton_megarachne");
        GameRegistry.registerTileEntity(TileEntityFossilMegarachne.class, PrehistoricNatureFossils.MODID + ":tileentityskeleton_megarachne");
    ...
    public String getDNACompat() {
        return "mobdnaPNlepidodendron:prehistoric_flora_megarachne";
    }

    @SideOnly(Side.CLIENT)
    @Override
    public void addInformation(ItemStack stack, World player, List<String> tooltip, ITooltipFlag advanced) {
        tooltip.add("When completed contains " + new TileEntityFossilMegarachne().getStages() + " part(s)");
    ...
    public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
        ...
        if (playerIn.getHeldItemMainhand().getItem() == Item.getItemFromBlock(this)) {
            if (!worldIn.isRemote) {
                ...
                if (tileEntity != null) {
                    if (tileEntity instanceof TileEntityFossilMegarachne) {
                        if (((TileEntityFossilMegarachne) tileEntity).getStages() >= newStage) {
