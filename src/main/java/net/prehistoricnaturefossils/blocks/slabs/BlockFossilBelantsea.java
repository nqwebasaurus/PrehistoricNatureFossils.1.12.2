...
public class BlockFossilBelantsea extends BlockSlabBase {

    public BlockFossilBelantsea() {
        setRegistryName(PrehistoricNatureFossils.MODID, "skeleton_belantsea");
        setTranslationKey("pf_skeleton_belantsea");
        GameRegistry.registerTileEntity(TileEntityFossilBelantsea.class, PrehistoricNatureFossils.MODID + ":tileentityskeleton_belantsea");
    ...
    public String getDNACompat() {
        return "mobdnaPNlepidodendron:prehistoric_flora_belantsea";
    }

    @SideOnly(Side.CLIENT)
    @Override
    public void addInformation(ItemStack stack, World player, List<String> tooltip, ITooltipFlag advanced) {
        tooltip.add("When completed contains " + new TileEntityFossilBelantsea().getStages() + " part(s)");
    ...
    public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
        ...
        if (playerIn.getHeldItemMainhand().getItem() == Item.getItemFromBlock(this)) {
            if (!worldIn.isRemote) {
                ...
                if (tileEntity != null) {
                    if (tileEntity instanceof TileEntityFossilBelantsea) {
                        if (((TileEntityFossilBelantsea) tileEntity).getStages() >= newStage) {
