...
public class BlockFossilMarrella extends BlockSlabBase {

    public BlockFossilMarrella() {
        setRegistryName(PrehistoricNatureFossils.MODID, "skeleton_marrella");
        setTranslationKey("pf_skeleton_marrella");
        GameRegistry.registerTileEntity(TileEntityFossilMarrella.class, PrehistoricNatureFossils.MODID + ":tileentityskeleton_marrella");
    ...
    public String getDNACompat() {
        return "mobdnaPNlepidodendron:prehistoric_flora_marrella";
    }

    @SideOnly(Side.CLIENT)
    @Override
    public void addInformation(ItemStack stack, World player, List<String> tooltip, ITooltipFlag advanced) {
        tooltip.add("When completed contains " + new TileEntityFossilMarrella().getStages() + " part(s)");
    ...
    public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
        ...
        if (playerIn.getHeldItemMainhand().getItem() == Item.getItemFromBlock(this)) {
            if (!worldIn.isRemote) {
                ...
                if (tileEntity != null) {
                    if (tileEntity instanceof TileEntityFossilMarrella) {
                        if (((TileEntityFossilMarrella) tileEntity).getStages() >= newStage) {
